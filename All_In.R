...
# !! Only this section needs to be edited before running !!
############################################################

# Set to the folder that contains your xlsx and gtf files.
# Using "." means the current working directory.
base_dir  <- "."

xlsx_file <- file.path(base_dir, "deg_results.xlsx")   # <- rename to match your file
gtf_file  <- file.path(base_dir, "gencode.annotation.gtf")  # <- rename to match your GTF

# All output will be written here (created automatically)
outdir <- file.path(base_dir, "lncRNA_Analysis_Output")

# Significance thresholds
padj_cutoff   <- 0.05   # FDR threshold
log2fc_cutoff <- 1.0    # minimum |log2 fold change| (~2-fold)

# Reproducibility seed
set.seed(1)

# Unified Up / Down / NS colour palette used across all plots
...
if (!file.exists(xlsx_file)) stop("Excel file not found: ", xlsx_file)

deg <- openxlsx::read.xlsx(xlsx_file, sheet = "deg_results")

if (!("Column1" %in% colnames(deg)))
  stop("Expected a column named 'Column1' containing ENSEMBL gene IDs.")

# Drop columns that are entirely NA (housekeeping artefacts from some exports)
deg <- deg %>%
  select(where(~ !all(is.na(.)))) %>%
  rename(gene_id = Column1) %>%
...
############################################################
# SECTION 16 — SAMPLE-LEVEL PLOTS  (optional)
#
# PCA, sample correlation heatmap, and top-variable-gene heatmap
# are generated only when a DESeq2 VST object (`vsd`) or a DESeq2
# dataset object (`dds`) is present in the R session.
#
# To enable these plots, run your DESeq2 pipeline first and keep
# the dds object:
#
#   dds <- DESeqDataSetFromMatrix(countData, colData, design = ~ condition)
#   dds <- DESeq(dds)
#   source("All_In.R")   # <- vsd will be created automatically from dds
#
# If neither vsd nor dds exists, this section is silently skipped.
############################################################

have_vsd <- exists("vsd")
have_dds <- exists("dds") && !have_vsd

if (!have_vsd && have_dds) {
...
if (have_vsd) {
  ...
  condition_col <- if ("condition" %in% colnames(meta)) "condition" else colnames(meta)[1]
  pca    <- prcomp(t(mat), scale. = FALSE)
  pct    <- round(100 * pca$sdev^2 / sum(pca$sdev^2), 1)
  pca_df <- data.frame(pca$x[, 1:2], meta)

  p_pca <- ggplot(pca_df,
                  aes(x = PC1, y = PC2, color = .data[[condition_col]])) +
    geom_point(size = 4) +
    labs(
      title = "PCA (VST normalised)",
      x = paste0("PC1 (", pct[1], "%)"),
      y = paste0("PC2 (", pct[2], "%)")
    )
  ggsave(file.path(outdir, "plots", "18_PCA_vst.png"),
         p_pca, width = 8, height = 6, dpi = 300)

  # Sample-to-sample Pearson correlation heatmap
  cors <- cor(mat, method = "pearson")
  png(file.path(outdir, "plots", "19_Sample_correlation_heatmap.png"),
      width = 2400, height = 2000, res = 300)
  pheatmap::pheatmap(cors, annotation_col = meta, annotation_row = meta)
  dev.off()

  # Heatmap of the top 500 most variable genes
  vars    <- matrixStats::rowVars(mat)
  top_idx <- order(vars, decreasing = TRUE)[seq_len(min(500L, length(vars)))]
  mat_var <- mat[top_idx, , drop = FALSE]
  png(file.path(outdir, "plots", "20_Heatmap_top_variable_genes.png"),
      width = 2400, height = 2600, res = 300)
  pheatmap::pheatmap(mat_var, annotation_col = meta, show_rownames = FALSE)
  dev.off()

  # Heatmap of the top 50 most significant lncRNAs
  rn_clean    <- sub("\\..*$", "", rownames(mat))
  top_lnc_ids <- deg_lnc %>%
    filter(!is.na(padj)) %>%
    arrange(padj) %>%
    head(50L) %>%
  ...
  keep_rows <- rn_clean %in% top_lnc_ids
  if (sum(keep_rows) >= 5) {
    mat_lnc           <- mat[keep_rows, , drop = FALSE]
    rownames(mat_lnc) <- rn_clean[keep_rows]
    png(file.path(outdir, "plots", "21_Heatmap_top50_lncRNAs.png"),
        width = 2400, height = 2600, res = 300)
    pheatmap::pheatmap(mat_lnc, annotation_col = meta, show_rownames = TRUE)
    dev.off()
  } else {
    message("Not enough top lncRNA IDs matched matrix row names — lncRNA heatmap skipped.")
  }

} else {
  message("Sections 18-21 skipped: no `vsd` or `dds` object found in the session.",
          "\nRun DESeq2 first and keep the dds object to enable these plots.")
}
